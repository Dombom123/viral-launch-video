...
import { Settings, RefreshCw, Rocket, Activity, Bug, ChevronRight, Zap } from 'lucide-react';
    ...
    <div className="h-screen w-full bg-[#09090b] text-zinc-200 flex flex-col overflow-hidden font-sans">
      ...
      {debugMode && (
        <div className="bg-amber-500/10 border-b border-amber-500/30 px-4 py-2 flex items-center justify-between animate-in slide-in-from-top duration-300">
          <div className="flex items-center gap-2 text-amber-400 text-sm">
            ...
            <span className="text-amber-500/70">— Click any step to jump • Ctrl+Shift+D to toggle</span>
          </div>
          <button 
      ...
      <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-6 shrink-0 bg-[#09090b]">
        <div className="flex items-center gap-3">
          ...
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-black shadow-lg transition-all ${debugMode ? 'bg-amber-400 shadow-amber-400/20' : 'bg-primary shadow-primary/20'}`}>
               <Rocket size={18} fill="currentColor" className="text-black" />
            </div>
            <span>ViralLaunch<span className="text-zinc-600">.ai</span></span>
          </div>
          <span className="px-2 py-0.5 bg-zinc-900 border border-zinc-800 text-zinc-500 text-[10px] uppercase tracking-wider font-bold rounded-full">Prototype</span>
        </div>
            ...
            <div>
              ...
              <div className="bg-zinc-900/50 rounded-xl border border-zinc-800 p-4 transition-colors hover:border-zinc-700">
                 <div className="flex items-center gap-3 mb-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center border border-zinc-700/50 ${debugMode ? 'bg-amber-500/20 text-amber-400' : 'bg-zinc-800 text-primary'}`}>
                       <Activity size={20} />
                    </div>
                    <div>
                       <div className="text-sm font-bold text-zinc-200">{useCachedRun ? 'Demo Project' : 'New Project'}</div>
                 ...
                 <div className="space-y-2 mt-4">
                    <div className="flex justify-between items-center text-[11px]">
                       ...
                       <span className="text-zinc-300 font-medium">{useCachedRun ? 'Loaded' : 'Awaiting Input'}</span>
                    </div>
                    <div className="flex justify-between items-center text-[11px]">
                  ...
                  {STEPS.map((step) => (
                     ...
                     >
                        <div className={`absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full border-2 transition-all duration-300
                           ${currentStep > step.id || (currentStep === step.id && step.id === STEPS.length) ? 'bg-primary border-primary' : 
                             currentStep === step.id ? 'bg-primary border-primary shadow-[0_0_10px_rgba(138,206,0,0.4)]' : 'bg-zinc-900 border-zinc-700'}
                        ...
                        `}>
                           {step.label}
                           {debugMode && (
                             <ChevronRight size={12} className="opacity-0 group-hover:opacity-100 transition-opacity text-amber-400" />
                           )}
                        </div>
                        {currentStep === step.id && (
                           <div className={`text-[10px] mt-1 font-medium ${step.id === STEPS.length ? 'text-primary' : 'text-primary animate-pulse'}`}>
                              {step.id === STEPS.length ? 'Complete' : 'In Progress...'}
                           </div>
                        )}
                     </div>
                  ))}
